...
func main() {
	...
	log.Fatal(http.ListenAndServe(":3000", devErrorMiddleware(mux)))
}
...
func devErrorMiddleware(app http.Handler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		defer func() {
			if err := recover(); err != nil {
				...
				fmt.Fprintf(w, "<h1>panic: %v</h1><pre>%s</pre>", err, getLinks(string(stack)))
			}
		}()
		app.ServeHTTP(w, r)
	}
...
func getLinks(stack string) string {
	...
	for lIdx, line := range lines {
		...
		lines[lIdx] = "\t<a href=\"/debug/?" + v.Encode() + "\">" + file + ":" + lineStr.String() + "</a>" + line[len(file)+2+len(lineStr.String()):]
	}
